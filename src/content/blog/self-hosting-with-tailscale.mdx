...
updated: "2026-09-13"
readingTime: "25min"
excerpt: "Running services at home and reaching them from anywhere, without forwarding a single port."
...
Run your services at home, put nothing on the public internet, and reach everything through your tailnet: install Tailscale on the server and on whatever you carry, and they're on one private network. Add a subnet router for devices that can't run Tailscale themselves, and Tailscale Serve when a service needs real HTTPS. Private by default, exposed only on purpose — that's the whole pattern.
</Callout>

<Diagram src="/images/blog/self-hosting-with-tailscale/remote-access.svg" alt="Remote access diagram showing encrypted tailnet links" />
*All traffic flows through an encrypted mesh overlay — no port forwarding, no public exposure. Your services are invisible to automated internet scans.*

<Diagram src="/images/blog/self-hosting-with-tailscale/security-layers.svg" alt="Layered security stack for self-hosting" />
*Defense in depth: identity, network, transport, and application layers each provide independent protection. If one layer fails, the others still hold.*
...
Docker's networking model doesn't play nicely with Tailscale out of the box. This is where I spent way too many hours debugging, so here is the short version.
...
### Do I have to forward ports to self-host?

No — and that's the point. Classic guides say "forward 443 to your box," which hands your server to every scanner on the internet. With Tailscale, services are reachable only from devices on your tailnet, and nothing of yours listens publicly at all.

### What's a subnet router?

One machine running Tailscale that advertises your LAN — say, `192.168.1.0/24` — into the tailnet. Devices that can't run Tailscale themselves, like printers and smart TVs, become reachable as if they were on it. The routes need approval in the admin console the first time.

### How do I get HTTPS for a service that never leaves the house?

Tailscale Serve. It puts a valid certificate on a tailnet service, so `https://jellyfin.your-tailnet.ts.net` works without public DNS, without Let's Encrypt, and without a single open port.

### Can I run Tailscale inside Docker?

Yes, two ways: `network_mode: host` so the container shares the machine's tailnet interface (simple, but port conflicts become yours), or a Tailscale sidecar container that other containers use as their gateway (more setup, more flexible).
