---
title: "Tailscale Explained: A Mesh VPN With No Open Ports"
date: "2026-02-17"
updated: "2026-09-13"
readingTime: "24min"
excerpt: "How Tailscale gets two machines talking directly through a router that should not allow it, and what it falls back to when NAT wins."
tags: ["tailscale", "vpn", "networking", "homelab", "wireguard"]
series: "homelab-networking"
seriesOrder: 2
---

import Diagram from '@/components/mdx/Diagram.astro';
import Callout from '@/components/mdx/Callout.astro';
import NerdCorner from '@/components/mdx/NerdCorner.astro';

The first time you SSH into your home server from a coffee shop **without** opening a port on your router, it's hard to see how it worked. There's no forwarded port, no static IP, and nothing listening on the public internet. This post is about what actually happened.

<Callout type="info" title="tl;dr">
Tailscale gives every device you own a WireGuard tunnel to every other device, with no ports open anywhere. A coordination server handles the parts nobody wants to hand-roll — identity, key exchange, NAT hole-punching — and only ever sees metadata, never your traffic. To the apps on your machines, the whole thing looks like one LAN. If your goal is reaching your own stuff from anywhere rather than hiding your browsing, it's the cleanest way to get there.
</Callout>

<Diagram src="/images/blog/tailscale-explained/tailnet-mesh.svg" alt="A simplified tailnet mesh diagram" />
...
<NerdCorner title="How WireGuard's cryptography works" subtitle="The Noise protocol handshake, step by step">

### How WireGuard key exchange actually works

WireGuard uses the [Noise Protocol Framework](https://noiseprotocol.org/), specifically the `Noise_IKpsk2` handshake pattern. Here's what that means in practice.
...
<Diagram src="/images/blog/tailscale-explained/wireguard-handshake.svg" alt="WireGuard Noise_IKpsk2 handshake sequence: the initiator sends its ephemeral and encrypted static keys, the responder computes DH1 and DH2 then replies with its ephemeral key, both mix all DH outputs plus the pre-shared key through HKDF to derive session keys, and encrypted transport follows.">
  The <code>Noise_IKpsk2</code> handshake: three Diffie-Hellman exchanges plus an optional PSK, all mixed via HKDF into forward-secret session keys — in a single round trip. <a href="/images/blog/tailscale-explained/wireguard-handshake.excalidraw">Open the editable Excalidraw source</a>.
</Diagram>
...
<Diagram src="/images/blog/tailscale-explained/control-plane-data-plane.svg" alt="Control plane vs data plane: the coordination server exchanges keys, ACLs, and peer discovery metadata with devices A, B, and C over dashed control-plane links, while the devices exchange actual encrypted WireGuard traffic directly peer-to-peer over solid data-plane links.">
  The coordination server only brokers metadata (keys, ACLs, peer discovery). Your actual traffic flows directly device-to-device over the data plane. <a href="/images/blog/tailscale-explained/control-plane-data-plane.excalidraw">Open the editable Excalidraw source</a>.
</Diagram>
...
Traditional VPNs and Tailscale solve different problems, and comparing them head-to-head only makes sense once that's clear.
...
<Diagram src="/images/blog/tailscale-explained/nat-traversal-derp.svg" alt="NAT traversal flow: both devices use a STUN server to learn their public IP and port, exchange those addresses through the coordination server, then send packets simultaneously to punch holes through their NATs for a direct peer-to-peer connection. When that fails due to symmetric NAT or blocked UDP, traffic falls back to an encrypted DERP relay.">
  STUN discovery and simultaneous sends punch a direct path through both NATs. When symmetric NAT or a UDP-blocking firewall gets in the way, encrypted traffic falls back to a DERP relay — which still can't read the WireGuard payload. <a href="/images/blog/tailscale-explained/nat-traversal-derp.excalidraw">Open the editable Excalidraw source</a>.
</Diagram>
...
Yes, in the practical sense: your devices sit on a private encrypted network. It's just not the legacy kind — there's no server everyone tunnels through, no port to forward, and no shared password. Each pair of devices gets its own WireGuard tunnel.

### Can Tailscale see my traffic?

No. Tailscale's coordination server is a control plane: it checks identity, hands out public keys, and tells devices how to find each other. Your actual traffic goes device-to-device over WireGuard. If even the control plane bothers you, Headscale lets you run that part yourself.

### What happens when NAT hole-punching fails?

Traffic falls back to DERP — Tailscale's relay network. Connections keep working and stay end-to-end encrypted; they just bounce through a relay instead of going direct, so they're slower. You can see whether a connection is direct or relayed in `tailscale status`.

### Do I need to open any ports?

No. Outbound connections from each device are enough, because the coordination server tells both sides how to reach each other and they punch holes simultaneously. Nothing has to listen on the public internet unless you deliberately use Funnel to expose a service.
...
- [Noise Protocol Framework](https://noiseprotocol.org/) — the handshake pattern WireGuard uses
