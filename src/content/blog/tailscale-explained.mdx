...
<NerdCorner title="WireGuard Cryptography Deep Dive" subtitle="How the Noise protocol handshake actually works">

### How WireGuard key exchange actually works

Here's where it gets nerdy (in a good way). WireGuard uses the **[Noise Protocol Framework](https://noiseprotocol.org/)** — specifically the `Noise_IKpsk2` handshake pattern. Don't worry, I'll translate.
...
<figure>
  <img src="/images/blog/tailscale-explained/wireguard-handshake.svg" alt="WireGuard Noise_IKpsk2 handshake sequence: the initiator sends its ephemeral and encrypted static keys, the responder computes DH1 and DH2 then replies with its ephemeral key, both mix all DH outputs plus the pre-shared key through HKDF to derive session keys, and encrypted transport follows." />
  <figcaption>The <code>Noise_IKpsk2</code> handshake: three Diffie-Hellman exchanges plus an optional PSK, all mixed via HKDF into forward-secret session keys — in a single round trip. <a href="/images/blog/tailscale-explained/wireguard-handshake.excalidraw">Open the editable Excalidraw source</a>.</figcaption>
</figure>
...
<figure>
  <img src="/images/blog/tailscale-explained/control-plane-data-plane.svg" alt="Control plane vs data plane: the coordination server exchanges keys, ACLs, and peer discovery metadata with devices A, B, and C over dashed control-plane links, while the devices exchange actual encrypted WireGuard traffic directly peer-to-peer over solid data-plane links." />
  <figcaption>The coordination server only brokers metadata (keys, ACLs, peer discovery). Your actual traffic flows directly device-to-device over the data plane. <a href="/images/blog/tailscale-explained/control-plane-data-plane.excalidraw">Open the editable Excalidraw source</a>.</figcaption>
</figure>
...
<figure>
  <img src="/images/blog/tailscale-explained/nat-traversal-derp.svg" alt="NAT traversal flow: both devices use a STUN server to learn their public IP and port, exchange those addresses through the coordination server, then send packets simultaneously to punch holes through their NATs for a direct peer-to-peer connection. When that fails due to symmetric NAT or blocked UDP, traffic falls back to an encrypted DERP relay." />
  <figcaption>STUN discovery and simultaneous sends punch a direct path through both NATs. When symmetric NAT or a UDP-blocking firewall gets in the way, encrypted traffic falls back to a DERP relay — which still can't read the WireGuard payload. <a href="/images/blog/tailscale-explained/nat-traversal-derp.excalidraw">Open the editable Excalidraw source</a>.</figcaption>
</figure>
