...
== Heaps
- *Definition:* A _complete binary tree_ satisfying the heap property:
...
- *Sets:* A collection of distinguishable elements.
	- Example: $A = {1, 4, 7, 2}$; membership denoted by $x in A$.
	- Operations: Union $A union B$, Intersection $A sect B$, Difference $A - B$.
== Factorials, Permutations, and Combinations
- *Factorial:*
  $ n! = n times (n-1) times dots.h.c times 1, quad 0! = 1. $
...
- *Big-O Notation ($O$) (Upper-Bound):*
  $ T(n) in O(f(n)) <=> exists c > 0, n_0 "such that" T(n) <= c f(n) "for all" n > n_0. $
- *Big-Omega ($Omega$) (Lower-Bound):*
  $ T(n) in Omega(g(n)) <=> exists c > 0, n_0 "such that" T(n) >= c g(n) "for all" n > n_0. $
- *Big-Theta ($Theta$) (Exact Growth Rate):*
  $ T(n) in Theta(h(n)) <=> T(n) "is both" O(h(n)) "and" Omega(h(n)). $
...
- *Complexity:*
    ...
    - Although the number of swaps is at most $O(n)$, the dominant cost is in the comparisons, making the overall time complexity $Theta(n^2)$.
#line(length: 100%)
...
_Example (Merge Sort):_
Here, $a = 2, b = 2, d = 1$ so that $2 = 2^1$, hence
$ T(n) = 2 T(frac(n, 2)) + n in Theta(n log n). $
== Merge Sort
